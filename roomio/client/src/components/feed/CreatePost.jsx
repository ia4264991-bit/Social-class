...
export default function CreatePost({ onPosted }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Guard: must have a session user
    if (!user?.id) {
      setPostError('You must be logged in to post.')
      return
    }

    // Guard: profile must exist in DB — otherwise foreign key will fail
    if (!profile?.id) {
      setPostError('Your profile is still loading. Please wait a moment and try again.')
      return
    }

    if (!content.trim() && !mediaFile) {
      setPostError('Write something or attach a file before posting.')
      return
    }
    ...
    if (mediaFile) {
      ...
      if (upErr) {
        ...
        setPostError('Media upload failed: ' + upErr.message)
        setLoading(false)
        return
      }
    ...
    try {
      ...
      const msg = err.response?.data?.error || err.message || 'Post failed'
      setPostError(msg)
  ...
  return (
    <div className="card p-4 mb-3">
      <div className="flex items-center gap-3 mb-3">
        <Avatar profile={profile} size={38} />
        <div>
          <div className="text-sm font-bold text-white">{profile?.full_name || 'Loading...'}</div>
          <div className="text-xs text-brand-green font-semibold">{profile?.hall || 'UCC'} · {profile?.level || 'CS'}</div>
        </div>
      </div>
      ...
      {mediaPreview && (
        <div className="relative mt-2 rounded-xl overflow-hidden">
          ...
          <button
            ...
            className="absolute top-2 right-2 bg-black/70 text-white rounded-full w-7 h-7 text-sm flex items-center justify-center press">
            ✕
          </button>
        </div>
      )}
      ...
      {postError && (
        <div className="mt-2 bg-red-900/30 border border-red-700/50 text-red-400 rounded-xl px-3 py-2 text-xs">
          {postError}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-dark-border">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => { setOpen(false); setPostError('') }}
            className="px-4 py-2 text-sm text-dark-muted rounded-xl hover:bg-dark-card2 press">
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            disabled={loading || (!content.trim() && !mediaFile) || !profile?.id}
